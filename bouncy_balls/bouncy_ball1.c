/* Implement bouncy ball with movement capabilities 
 * I rewrote the C++ "SDL2 movement tutorial" from Lazyfoo
 * to plain C
...
void bounce(SDL_Rect *bRect)
{
    ...
    if((bRect->y < 0) || (bRect->y + bRect->h > (SCREEN_HEIGHT - floorOffset)))
    {
        /* Change direction */
        ac = ac * -1;
...
void move(SDL_Rect *bRect)
{
    ...
    /* Detect wall */
    if((bRect->x < 0 ) || (bRect->x + bRect->w > SCREEN_WIDTH)){
        bRect->x -= velX;
    }
    ...
    if(( bRect->y < 0 ) || (bRect->y + bRect->h > SCREEN_HEIGHT )){
        bRect->y -= velY;
    }
}
